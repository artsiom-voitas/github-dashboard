...
function SearchPagination(props) {
  ...
  return (
    <Pagination className="mt-4 d-flex justify-content-center">
      ...
      {currentPage < 5 ? null : <Pagination.Ellipsis />}
      {currentPage === pagesAmount ? (
        <Pagination.Item href={setUrl(currentPage - 4)}>
          {currentPage - 4}
        </Pagination.Item>
      ) : null}
      ...
      {currentPage > 3 ? (
        <Pagination.Item href={setUrl(currentPage - 2)}>
          {currentPage - 2}
        </Pagination.Item>
      ) : null}
      {currentPage > 2 ? (
        <Pagination.Item href={setUrl(currentPage - 1)}>
          {currentPage - 1}
        </Pagination.Item>
      ) : null}
      ...
      {currentPage + 4 > pagesAmount ? null : <Pagination.Ellipsis />}
      {currentPage + 3 > pagesAmount ? null : (
        <Pagination.Item href={setUrl(pagesAmount)}>
          {pagesAmount}
        </Pagination.Item>
      )}
      {currentPage === pagesAmount ? (
